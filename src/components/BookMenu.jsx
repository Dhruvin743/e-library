	...
	useEffect(() => {
		const pull = async () => {
			if (searchData) {
				...
				const data = await fetch("http://localhost:2323/home", { credentials: "include" });
				const res = await data.json();
				if (res.code) {
					nav("/login");
				} else {
				...
				{bookList.map((v, i) => {
					return (
						<Card
							...
							imgsrc={v.cover_i}
							cardtitle={v.title}
							bookid={v.key.split("/")[v.key.split.length]}
							isNew={"d-none"}
						/>
					);
				})}
			</div>
			<div className='w-100 p-2 position-absolute bottom-0'>
