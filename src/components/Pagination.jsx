...
import pageStyle from "./stylesheets/pagination.module.css";
...
export const Widget = () => {
	...
	const isActive = (page) => (activePage === page ? pageStyle.active : "");
	...
	return (
		<ul className={pageStyle.pagination}>
			...
			{pages.map((page) => (
				<button className={isActive(page)} onClick={() => onChange(page)} key={page}>
					{page + 1}
				</button>
			))}
			{activePage < pageCount - 4 && <span>...</span>}
			{pageCount > 5 && (
				<button className={isActive(pageCount - 1)} onClick={() => onChange(pageCount - 1)}>
					{pageCount}
				</button>
			)}
		</ul>
